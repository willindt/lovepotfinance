...
type PublicFarmData = {
  ...
  multiplier: string
  depositfee: string
  loveperblock: SerializedBigNumber
  canharvest: boolean
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, lovePerBlock, startBlockHarvest] =
    pid || pid === 0
  ...
  const LovePerBlock = lovePerBlock? new BigNumber(lovePerBlock).div(BIG_TEN.pow(18)) : BIG_ZERO
  ...
  return {
    ...
    loveperblock: LovePerBlock.toJSON(),
    canharvest
  }
}
